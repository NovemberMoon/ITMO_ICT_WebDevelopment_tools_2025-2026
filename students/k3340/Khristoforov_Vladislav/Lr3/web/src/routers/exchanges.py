...
async def create_exchange(exchange: ExchangeRequestCreate, session: AsyncSession = Depends(get_session), current_user: User = Depends(get_current_user)):
    exchange.requester_id = current_user.id
    
    requested_book = await session.get(Book, exchange.requested_book_id)
    ...
    # Валидация предложенной взамен книги (если она указана)
    if exchange.offered_book_id:
        offered_book = await session.get(Book, exchange.offered_book_id)
...
async def read_exchanges(offset: int = 0, limit: int = Query(default=100, le=100), session: AsyncSession = Depends(get_session)):
    result = await session.exec(select(ExchangeRequest).where(ExchangeRequest.is_deleted == False).offset(offset).limit(limit))
    return result.all()

@exchanges_router.get("/{exchange_id}", response_model=ExchangeRequestPublic)
async def read_exchange(exchange_id: int, session: AsyncSession = Depends(get_session)):
    exchange = await session.get(ExchangeRequest, exchange_id)
    if not exchange:
...
async def update_exchange(
    ...
    db_exchange = await session.get(ExchangeRequest, exchange_id)
    if not db_exchange or db_exchange.is_deleted:
        raise HTTPException(status_code=404, detail="Exchange request not found")
        
    requested_book = await session.get(Book, db_exchange.requested_book_id)
    
    # Проверка прав: менять статус могут только участники или админ
    ...
    requested_book = await session.get(Book, db_exchange.requested_book_id)
    offered_book = await session.get(Book, db_exchange.offered_book_id) if db_exchange.offered_book_id else None

    # Транзакция бронирования книг при одобрении заявки
    ...
    # Транзакция отмены: освобождаем книги из брони
    ...
    # Транзакция успешного завершения: физическая смена владельцев
...
async def delete_exchange(
    ...
    if exchange.status == ExchangeStatus.ACCEPTED:
        requested_book = await session.get(Book, exchange.requested_book_id)
        offered_book = await session.get(Book, exchange.offered_book_id) if exchange.offered_book_id else None
        if requested_book: requested_book.is_available = True
        if offered_book: offered_book.is_available = True
        session.add(requested_book)
        if offered_book: session.add(offered_book)
...
async def create_review(review: ReviewCreate, session: AsyncSession = Depends(get_session), current_user: User = Depends(get_current_user)):
    review.author_id = current_user.id

    exchange = await session.get(ExchangeRequest, review.exchange_id)
    if not exchange or exchange.is_deleted:
        raise HTTPException(status_code=404, detail="Exchange not found")
        
    # Защита от накруток: отзывы пишут только после реального обмена
    ...
    # Защита от спама: отзыв могут оставить только участники сделки
...
async def read_reviews(offset: int = 0, limit: int = Query(default=100, le=100), session: AsyncSession = Depends(get_session)):
    result = await session.exec(select(Review).where(Review.is_deleted == False).offset(offset).limit(limit))
    return result.all()

@reviews_router.get("/{review_id}", response_model=ReviewPublic)
async def read_review(review_id: int, session: AsyncSession = Depends(get_session)):
...
async def update_review(review_id: int, review_data: ReviewUpdate, session: AsyncSession = Depends(get_session), current_user: User = Depends(get_current_user)):
...
async def delete_review(review_id: int, session: AsyncSession = Depends(get_session), current_user: User = Depends(get_current_user)):
    review = await session.get(Review, review_id)
    if not review:
        raise HTTPException(status_code=404, detail="Review not found")
