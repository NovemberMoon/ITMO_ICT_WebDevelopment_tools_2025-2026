...
| **Финальная версия**        | Реализация JWT, замков безопасности (RBAC) и смены паролей         | [Коммит Финальной версии](https://github.com/NovemberMoon/ITMO_ICT_WebDevelopment_tools_2025-2026/commit/bdc88df416937d669419f142b720f7acb3626799) |

## 2. Архитектура и стек технологий

Программное решение спроектировано по модульному принципу, разделяя доменные модели, логику подключения к базе данных, сквозную безопасность и конечные точки API.
...
| **СУБД**          | PostgreSQL                               | Хранение реляционных структурированных данных приложения.                                                |
| **ORM / Схемы**   | SQLModel (v0.0.38)                       | Единое описание таблиц базы данных и схем валидации Pydantic, исключающее дублирование описания полей.   |
...
# Если используем SQLite, нужен специальный аргумент. Для Postgres он вызывает ошибку.
connect_args = {"check_same_thread": False} if settings.DATABASE_URL.startswith("sqlite") else {}

# Создаем подключение к БД с правильными аргументами
...
3. **Конфигурация переменных окружения:**
    ...
    ```bash
    DB_URL=postgresql://postgres:password@localhost:5432/bookcrossing_db
    SECRET_KEY=your_secure_secret_key_string
    ALGORITHM=HS256
    ACCESS_TOKEN_EXPIRE_MINUTES=30
    ```

4. **Применение миграций схемы к PostgreSQL:**
...
В рамках выполнения лабораторной работы №1 было успешно спроектировано и реализовано серверное приложение. Были освоены современные практики построения быстрых API-интерфейсов на фреймворке FastAPI, организации реляционного хранения данных с помощью библиотеки декларативного описания моделей SQLModel (надстройка над SQLAlchemy), а также принципы развертывания миграций баз данных с помощью Alembic. Интеграция JWT-авторизации позволила гибко настроить разграничение прав доступа и защитить ключевые бизнес-процессы системы обмена книгами.
